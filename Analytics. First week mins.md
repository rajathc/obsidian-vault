...
eligible_memories AS (
  SELECT md.user_id, md.memory_date, md.started_at, md.finished_at, fm.first_date
  FROM memory_data md
  JOIN first_memory fm ON md.user_id = fm.user_id
  WHERE fm.first_date <= '2025-05-20'
),

first_week_durations AS (
  SELECT
    user_id,
    SUM(EXTRACT(EPOCH FROM finished_at - started_at)) / 60 AS total_minutes
...
final_status AS (
  SELECT
    ...
    CASE 
      ...
      ELSE 'not_churned'
    END AS status
  FROM first_week_durations fwd
  JOIN gap_flags gf ON fwd.user_id = gf.user_id
)

SELECT
  status,
  COUNT(*) AS user_count,
  ROUND(AVG(total_minutes)::numeric, 2) AS avg_first_week_minutes,
